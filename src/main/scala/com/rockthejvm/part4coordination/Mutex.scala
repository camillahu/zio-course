      ...
      override def acquire = ZIO.uninterruptibleMask { restore =>
        Promise.make[Nothing, Unit].flatMap { signal =>
          ...
          state.modify {
            ...
            case State(true, waiting) => restore(signal.await).onInterrupt(cleanup) -> State(true, waiting.enqueue(signal))
          }.flatten
        }
      }
        ...
        state.modify {
          ...
          case State(true, waiting) =>
            if (waiting.isEmpty) ZIO.unit -> unlocked
              ...
              first.succeed(()).unit -> State(true, rest)
            }
        }.flatten
    }
  }
...
object MutextPlayground extends ZIOAppDefault {

  def workInCriticalRegion(): UIO[Int] =
    // ZIO.sleep(1.second) *> Random.nextIntBounded(100) (same as below)
...
//    })
  ...
  def createInterruptingTask(id: Int, mutex: Mutex): UIO[Int] = {
    if (id % 2 == 0)
     ...
     // _ <- ZIO.sleep(2500.millis) *> ZIO.succeed(s"interrupting task $id").debugThread *> fib.interrupt (same as below)
