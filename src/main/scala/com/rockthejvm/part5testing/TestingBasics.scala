...
object MyFirstEffectTestSpec extends ZIOSpecDefault {
  def spec = test("First Effect Test") {
    ...
    //testing ZIO's doesn't work with assertTrue

    /*
      Assertion examples:
      - Assertion.equalTo => tests for equality = the most general assertion
      - Assertion.assertion => tests any truth value
      - Assertion.fail/failsCause => expects the effect to fail with the exact failure/cause you specify
...
object ASuiteSpec extends ZIOSpecDefault {
  def spec = suite("Full suite of tests")(
    // pass multiple tests as args
    test("simple test") {
      assertTrue(1 + 3 == 4)
    },
    test("a second test") {
      assertZIO(ZIO.succeed("Scala"))(Assertion.hasSizeString(Assertion.equalTo(5))
        && Assertion.startsWithString("S"))
    },
    //sub-suites
    suite("a nested suite")(
      test("a nested test") {
        assert(List(1,2,3))(Assertion.isNonEmpty
          && Assertion.hasSameElements(List(3,2,1)))
      },
